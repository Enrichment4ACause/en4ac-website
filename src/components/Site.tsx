...
export function Header({page = 'home'}: {page?: SitePage}){
  ...
  const section=(hash: string)=>page==='home'?hash:`/${hash}`;
  return <header className="header"><Logo/><nav className={open?'nav open':'nav'} aria-label="Main navigation"><a href={section('#about')} onClick={close}>Our purpose</a><a href={section('#projects')} onClick={close}>The projects</a><a href={section('#get-involved')} onClick={close}>For nonprofits</a><a href="/blog" aria-current={page==='blog'?'page':undefined} onClick={close}>Blog</a></nav><JoinButton/><button className="mobile-menu" aria-label="Toggle navigation" aria-expanded={open} onClick={()=>setOpen(!open)}>{open?<X/>:<Menu/>}</button></header>
}
